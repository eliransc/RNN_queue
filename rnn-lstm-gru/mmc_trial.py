...
def generate_mmc(capacity, num_servers, transion_matrix, util_lower=0.8, util_upper=0.8000000001, arrival_dist=4, ser_dist=4):
    '''
    ...
    for server in range(num_servers):
        ...
        A_arrival = A_arrival * b1[server]  # arrival_rate
        ...
        ex = np.random.uniform(ex_lower[server], ex_upper[server])

        A_service = A_service / ex

        moms_service = np.array(compute_first_n_moments(s_service, A_service, 10)).flatten()

        rho = x1[server] * moms_service[0] / capacity[server]
        print('Rho in station  {} is: {}' .format(server, rho))
    ...
    return (arrival_dist_list, ser_dist_list, (s_arrival_list, A_arrival_list, moms_arrive_list),
            (s_service_list, A_service_list, moms_service_list) ,x1)
...
class GG1:
    ...
    def __init__(self, capacities, num_classes, model_input, transion_matrix, num_stations):
        self.num_stations = num_stations
        self.transion_matrix = transion_matrix
        self.num_classes = num_classes
        self.customer_counter = np.zeros(num_stations)
        self.capacities = capacities
        self.env = simpy.Environment()
        self.patient_counter = 0
        self.server = [simpy.PriorityResource(self.env, capacity=capacity) for capacity in capacities]
        arrival_dist, ser_dist, arrival_dist_params, ser_dist_params, x1 = model_input  # self.get_gg1_input()
        self.arrival_dist = arrival_dist
        self.ser_dist = ser_dist
        self.arrival_dist_params = arrival_dist_params
        self.ser_dist_params = ser_dist_params
        self.arrival_rate = [rate for rate in x1]
        self.num_cust_sys = np.zeros(num_stations)
        self.num_cust_durations = [np.zeros(500) for ind in range(num_stations)]
        self.last_event_time = np.zeros(num_stations)
        self.last_departure = np.zeros(num_stations)
        self.event_log = pd.DataFrame([], columns=['Customer_id', 'Priority', 'Arrival_time', 'Departure_time', 'Station'])
        self.queueing_time = [{} for ind in range(num_stations)]

    def run(self):

        for ind in range(self.capacities.shape[0]):
            self.env.process(self.customer_arrivals(ind))
    ...
    def service(self, customer, server_id):
        ...
        with self.server[server_id].request(priority=customer.priority) as req:  # priority=priority
            yield req

            q_time = self.env.now - arrival_time

            if customer.priority in self.queueing_time[server_id].keys():
                self.queueing_time[server_id][customer.priority].append(q_time)
            else:
                self.queueing_time[server_id][customer.priority] = [q_time]
            ...
            if self.ser_dist == 1:
                ...
                inter_ser = SamplesFromPH(ml.matrix(self.ser_dist_params[0][server_id]), self.ser_dist_params[1][server_id], 1)[0]
                yield self.env.timeout(inter_ser)

            if customer.id > g.warm_up_arrivals:
                g.inter_departure_time.append(self.env.now - self.last_departure[server_id])

            self.last_departure[server_id] = self.env.now
            ...
            if customer.id > g.warm_up_arrivals:
                ...
                self.event_log.loc[curr_ind, 'Station'] = server_id

            g.service_times.append(inter_ser)

            tot_time = self.env.now - self.last_event_time[server_id]
            self.num_cust_durations[server_id][int(self.num_cust_sys[server_id])] += tot_time
            self.num_cust_sys[server_id] -= 1
            self.last_event_time[server_id] = self.env.now


            # print('service completion at {} station {}, num of customer is {}'.format(self.env.now, server_id, self.num_cust_sys[server_id]))

            next_station = np.random.choice(self.transion_matrix[server_id,:].shape[0]+1,
                                            1, p=np.append(self.transion_matrix[server_id,:],
                                                           1-np.sum(self.transion_matrix[server_id, :])))[0]

            if next_station < self.transion_matrix[server_id,:].shape[0]:
                # print('Customer {} redirects from station {} to station {} at {} ' .format(customer.id, server_id, next_station, self.env.now))
                tot_time = self.env.now - self.last_event_time[next_station]
                self.num_cust_durations[next_station][int(self.num_cust_sys[next_station])] += tot_time
                self.num_cust_sys[next_station] += 1
                self.last_event_time[next_station] = self.env.now
                self.env.process(self.service(customer, next_station))
            else:
                pass
                # print('Customer {} leaves the system from station {} at {}' .format(customer.id, server_id, self.env.now))



    def customer_arrivals(self, server_id):

        while True:

            if self.arrival_dist[server_id] == 1:
                a_arrive = self.arrival_dist_params[server_id][0]
                b_arrive = self.arrival_dist_params[server_id][1]
                inter_arrival = np.random.uniform(a_arrive, b_arrive)
                yield self.env.timeout(inter_arrival)

            elif self.arrival_dist == 2:
                shape = self.arrival_dist_params[server_id][0]
                scale = self.arrival_dist_params[server_id][1]
                inter_arrival = np.random.gamma(shape, scale)
                yield self.env.timeout(inter_arrival)

            elif self.arrival_dist == 3:
                mu = self.arrival_dist_params[server_id][0]
                sig = self.arrival_dist_params[server_id][1]
                inter_arrival = np.max([0.000001, np.random.normal(mu, sig)])
                yield self.env.timeout(inter_arrival)

            else:
                inter_arrival = SamplesFromPH(ml.matrix(self.arrival_dist_params[0][server_id]), self.arrival_dist_params[1][server_id], 1)[0]
                # print(inter_arrival)
                yield self.env.timeout(inter_arrival)



            self.customer_counter[server_id] += 1

            arrival_time = self.env.now
            customer = Customer(self.customer_counter[server_id], self.num_classes, arrival_time)

            # print('Arrival of {} customer in station {} at {} ' .format(customer.id, server_id, self.env.now))
            ...
            self.last_event_time[server_id] = self.env.now
            self.num_cust_sys[server_id] += 1
            # print('station {}, num of customer is {} at {}'.format(server_id, self.num_cust_sys[server_id], self.env.now))
            self.env.process(self.service(customer, server_id))
...
def main(args):
    ...
    for ind in tqdm(range(args.num_iterations)):
        ...
        model_inputs = generate_mmc(capacity, num_stations, transion_matrix)

        # for num_classes in tqdm(range(1, args.max_num_classes + 1)):

        start_time = time.time()
        gg1 = GG1(capacity, num_classes, model_inputs, transion_matrix, num_stations)
        gg1.run()
        arrivals_rates = model_inputs[-1]
        steady_state = [gg1.num_cust_durations[ind] / np.sum(gg1.num_cust_durations[ind]) for ind in range(num_stations)]
        dist_dict = {1: 'uniform', 2: 'gamma', 3: 'normal', 4: 'PH'}
        # arrival_rate = 1 / gg1.arrival_dist_params[2][0]
        arrival_dist, ser_dist, arrival_dist_params, ser_dist_params, x1 = model_inputs
        rhos = [arrivals_rates[ind]*ser_dist_params[2][ind][0]/gg1.capacities[ind] for ind in range(num_stations)]
        vals = np.arange(10)
        theoretical_steady = [(1-rhos[ind])*rhos[ind]**vals for ind in range(num_stations)]
        print('The capacity is: ', gg1.capacities)
        print('rhos are: ', rhos)
        print('Theoretical steady: ', theoretical_steady)
        print('Sim steady station 0: ', steady_state[0][:10])
        print('Sim steady station 1: ', steady_state[1][:10])
        print("--- %s seconds the %d th iteration ---" % (time.time() - start_time, ind))

        model_num = np.random.randint(1, 1000000)

        curr_path = args.log_event_path + '_' + 'capacity_' + str(capacity) + '_' + 'priority' + '_' + str(
            num_classes) + '_' + str(model_num) + '.pkl'
...
def parse_arguments(argv):
    ...
    parser.add_argument('--log_event_path', type=str, help='case number in my settings',
                        default='/scratch/eliransc/RNN_data/log_event')
    parser.add_argument('--is_corr', type=bool, help='should we keep track on inter departure', default=True)
    parser.add_argument('--waiting_pkl_path', type=bool, help='the path of the average waiting time',
                        default='./pkl/waiting_time')
